...
type Phase = 'questionnaire' | 'script' | 'rapport' | 'historique'
...
export function AuditClient({ audits, prefill, commercialDefaut, tableManquante, auditIdOuvert }: AuditClientProps) {
  ...
  const [leadId, setLeadId] = useState<string | null>(prefill?.lead_id || null)
  ...
  // Prefill : lead rattaché (?lead=) puis, à défaut, ancien relais localStorage
  useEffect(() => {
    const appliquer = (d: Prefill) => {
    ...
    if (prefill) appliquer(prefill)
    try {
      const pf = localStorage.getItem('ll_prefill_audit')
      if (pf) {
        if (!prefill) appliquer(JSON.parse(pf))
        localStorage.removeItem('ll_prefill_audit')
      }
    } catch {}
    setCommercialNom((v) => v || commercialDefaut)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  /** Recharge un audit enregistré dans le formulaire. */
  function ouvrirAudit(a: any, allerAuRapport = true) {
    setAuditId(a.id)
    setLeadId(a.lead_id || null)
    setClientId(a.client_id || null)
    setEtabNom(a.etablissement || '')
    setEtabType(a.etab_type || '')
    setConvention(a.convention || '')
    setEffectif(a.effectif != null ? String(a.effectif) : '')
    setCommercialNom(a.commercial_nom || '')
    setDuerpState(a.duerp_etat || '')
    setFormations({
      haccp: { total: String(a.axes?.haccp?.total ?? ''), formed: String(a.axes?.haccp?.formed ?? ''), linked: false },
      duerp: { total: String(a.axes?.duerp?.total ?? ''), formed: String(a.axes?.duerp?.formed ?? ''), linked: false },
      sst: { total: String(a.axes?.sst?.total ?? ''), formed: String(a.axes?.sst?.formed ?? ''), linked: false },
      incendie: { total: String(a.axes?.incendie?.total ?? ''), formed: String(a.axes?.incendie?.formed ?? ''), linked: false },
    })
    setOpcoKnown(a.opco || null)
    setCompteActif(a.compte_actif || null)
    setEngagement(a.engagement || null)
    setContactNom(a.contact_nom || '')
    setContactEmail(a.contact_email || '')
    setContactTel(a.contact_telephone || '')
    setProjectionPeriode(a.projection_periode || '')
    setRdvDate(a.rdv_date || '')
    setRdvHeure(a.rdv_heure || '')
    setQuickNotes(a.notes || '')
    if (allerAuRapport) setPhase('rapport')
  }

  // Ouverture directe via ?audit=<id>
  useEffect(() => {
    if (!auditIdOuvert) return
    const a = audits.find((x) => x.id === auditIdOuvert)
    if (a) ouvrirAudit(a)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [auditIdOuvert])
  ...
  function handleReset() {
    ...
    setAuditId(null); setLeadId(prefill?.lead_id || null); setClientId(null)
    setContactNom(''); setContactEmail(''); setContactTel(''); setProjectionPeriode('')
    setRdvDate(''); setRdvHeure(''); setQuickNotes('')
  }

  /** Enregistre l'audit en base (création ou mise à jour). */
  async function handleSave() {
    if (!etabNom.trim()) {
      toast('error', "Renseignez le nom de l'établissement avant d'enregistrer")
      setPhase('questionnaire')
      return
    }
    setSaving(true)
    const axes: Record<string, { total: number; formed: number }> = {}
    for (const k of ['haccp', 'duerp', 'sst', 'incendie']) {
      axes[k] = { total: parseInt(formations[k].total) || 0, formed: parseInt(formations[k].formed) || 0 }
    }
    const res = await saveAuditAction({
      id: auditId,
      lead_id: leadId,
      client_id: clientId,
      ...
      axes,
      duerp_etat: duerpState,
      score: scoreData.score,
      couverture: scoreData.coverage,
      opco: opcoKnown,
      ...
      projection_periode: projectionPeriode,
      rdv_date: rdvDate,
      rdv_heure: rdvHeure,
      notes: quickNotes,
    })
    ...
    setAuditId((res.data as any).id)
    toast('success', auditId ? 'Audit mis à jour' : 'Audit enregistré')
    router.refresh()
  }
  ...
  async function handleCreerLead(id: string) {
    setBusyId(id)
    const res = await creerLeadDepuisAuditAction(id)
    setBusyId(null)
    if (!res.success) { toast('error', res.error || 'Erreur'); return }
    toast('success', 'Lead créé depuis l\'audit')
    router.push(`/dashboard/leads?lead=${(res.data as any).id}`)
  }
  ...
  return (
    <div className="animate-fade-in max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-heading font-bold text-surface-900 tracking-heading">Audit de conformité</h1>
          <p className="text-surface-500 mt-1 text-sm">
            {auditId ? 'Audit enregistré — les modifications écrasent la version en base' : 'Diagnostic réglementaire en 3 phases'}
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="secondary" onClick={handleReset} icon={<RotateCcw className="h-4 w-4" />}>Nouvel audit</Button>
          <Button onClick={handleSave} isLoading={saving} icon={<Save className="h-4 w-4" />}>
            {auditId ? 'Mettre à jour' : 'Enregistrer'}
          </Button>
        </div>
      </div>
      ...
      {tableManquante && (
        <div className="card p-4 mb-5 border-warning-200 bg-warning-50/50 flex items-start gap-3">
          ...
          <div className="text-sm text-surface-700">
            <span className="font-medium">Historique indisponible.</span> Appliquez la migration
            <code className="mx-1 px-1.5 py-0.5 rounded bg-white border border-warning-200 text-xs">113_audits_conformite.sql</code>
            dans Supabase pour enregistrer les audits.
          </div>
        </div>
      )}
      ...
      {leadId && (
        <div className="mb-5 flex items-center gap-2 text-sm">
          ...
          <span className="text-surface-500">Audit rattaché au lead</span>
          <Link href={`/dashboard/leads?lead=${leadId}`} className="font-medium text-brand-600 hover:underline">
            {etabNom || 'Voir la fiche'}
          </Link>
        </div>
      )}
          ...
          {audits.map(a => (
            <div key={a.id} className="card p-4 flex items-center gap-4">
              <div
                ...
                style={{ backgroundColor: `${scoreColor(Number(a.score) || 0)}1a`, color: scoreColor(Number(a.score) || 0) }}
              >
                {a.score != null ? Number(a.score).toFixed(1) : '--'}
              </div>
              <div className="min-w-0 flex-1">
                <div className="text-sm font-heading font-semibold text-surface-900 truncate">{a.etablissement}</div>
                <div className="text-xs text-surface-500">
                  {formatDate(a.created_at)}
                  {a.commercial_nom ? ` · ${a.commercial_nom}` : ''}
                  {a.effectif ? ` · ${a.effectif} salariés` : ''}
                  {a.couverture != null ? ` · couverture ${a.couverture} %` : ''}
                </div>
              </div>
              {a.rdv_date && (
                <Badge variant="info" className="hidden sm:inline-flex">RDV {formatDate(a.rdv_date)}</Badge>
              )}
              {a.lead_id ? (
                <Link href={`/dashboard/leads?lead=${a.lead_id}`} className="hidden sm:block text-xs text-brand-600 hover:underline">Lead</Link>
              ) : (
                <Button variant="ghost" size="sm" onClick={() => handleCreerLead(a.id)} isLoading={busyId === a.id} icon={<UserPlus className="h-4 w-4" />}>
                  Créer le lead
                </Button>
              )}
              <Button variant="secondary" size="sm" onClick={() => ouvrirAudit(a)} icon={<Eye className="h-4 w-4" />}>Ouvrir</Button>
              <button
