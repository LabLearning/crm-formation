...
export function FormationForm({ formation, onSuccess, onCancel }: FormationFormProps) {
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
      ...
      {/* Branches métier & site vitrine */}
      <div className="pt-4 mt-2 border-t border-surface-100 space-y-3">
        <div className="text-sm font-semibold text-surface-900">Branches métier & site vitrine</div>
        <div>
          <div className="text-xs text-surface-500 mb-2">Branches ciblées par cette formation</div>
          <div className="grid sm:grid-cols-2 gap-2">
            {BRANCHES_BASE.map((b) => (
              <label key={b.slug} className="flex items-center gap-2 text-sm text-surface-700 rounded-lg border border-surface-200 px-3 py-2 hover:border-brand-300 cursor-pointer">
                <input type="checkbox" name="branches" value={b.slug} defaultChecked={((formation as any)?.branches || []).includes(b.slug)} className="rounded border-surface-300" />
                {b.label}
              </label>
            ))}
          </div>
        ...
        <label className="flex items-center gap-2 text-sm text-surface-700">
          <input type="checkbox" name="est_transverse" value="true" defaultChecked={(formation as any)?.est_transverse} className="rounded border-surface-300" />
          <span>Transverse — proposée à <strong>toutes</strong> les branches (hygiène, sécurité, management…)</span>
        </label>
        <label className="flex items-center gap-2 text-sm text-surface-700">
