        ...
        <div className="flex flex-wrap gap-2 mb-5">
          {poeis.map((p) => (
            <button key={p.id} onClick={() => setSel(p.id)}
              ...
              {clientNom(p) || p.numero || p.formation?.intitule}
            </button>
          ))}
        </div>
      )}
      ...
      {poei && (
        <div className="card p-4 mb-5">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              <div className="font-heading font-semibold text-surface-900">{clientNom(poei) || poei.formation?.intitule || 'Projet POEI'}</div>
              <div className="text-xs text-surface-500 mt-0.5 flex items-center gap-2">
                ...
                {poei.numero && <span className="text-surface-400">· {poei.numero}</span>}
              </div>
            </div>
