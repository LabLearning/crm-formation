...
export function SimulateurPriseEnCharge({ formations }: { formations: FormationSimu[] }) {
  ...
  const Pastille = ({ n }: { n: number }) => (
    <div className="flex items-center gap-2 mb-5">
      {[1, 2, 3, 4].map((i) => (
        <span key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i <= n ? 'w-8 bg-[#195144]' : 'w-4 bg-[#195144]/15'}`} />
      ))}
    </div>
  )
  ...
  if (envoye) {
    return (
      <div className="rounded-3xl bg-white ring-1 ring-black/5 p-8 md:p-10 text-center">
        <span className="mx-auto h-14 w-14 rounded-2xl bg-[#195144]/10 flex items-center justify-center"><CheckCircle2 className="h-7 w-7 text-[#195144]" /></span>
    ...
    <div className="rounded-3xl bg-white ring-1 ring-black/5 p-6 md:p-8">
        ...
        <div>
          ...
          <div className="mt-4 flex gap-2">
            <input value={siret} onChange={(e) => setSiret(e.target.value)} inputMode="numeric"
              ...
              className="flex-1 rounded-xl border border-[#E7E5E4] px-4 py-3 text-sm focus:outline-none focus:border-[#195144]/50" />
            <button onClick={chercherSiret} disabled={chargeSiret}
              className="shrink-0 inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-[#195144] text-white text-sm font-semibold hover:bg-[#123f34] disabled:opacity-50">
          ...
          {entreprise && (
            <div className="mt-4 rounded-2xl bg-[#195144]/5 border border-[#195144]/15 p-4 flex items-start gap-3">
              <Building2 className="h-5 w-5 text-[#195144] shrink-0 mt-0.5" />
              <div className="text-sm">
                ...
                {entreprise.brancheSuggeree && (
                  <div className="text-[#195144] mt-1 font-medium">
          ...
          <div className="mt-5 flex items-center justify-between">
            ...
            <button onClick={() => setEtape(2)} disabled={!entreprise}
              className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[#195144] text-white text-sm font-semibold hover:bg-[#123f34] disabled:opacity-40">
            ...
            {BRANCHES.map((b) => (
              <button key={b.slug} onClick={() => { setBranche(b.slug); setFormationId(null) }}
                className={`text-left rounded-2xl border p-4 transition-colors ${
                  branche === b.slug ? 'border-[#195144] bg-[#195144]/5' : 'border-[#E7E5E4] hover:border-[#195144]/40'
                }`}>
            ...
            {formationsBranche.map((f) => (
              <button key={f.id} onClick={() => setFormationId(f.id)}
                className={`w-full text-left rounded-2xl border px-4 py-3 transition-colors ${
                  formationId === f.id ? 'border-[#195144] bg-[#195144]/5' : 'border-[#E7E5E4] hover:border-[#195144]/40'
                }`}>
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            {[2, 4, 6, 8, 10].map((n) => (
              <button key={n} onClick={() => setStagiaires(n)}
                className={`h-9 w-9 rounded-full text-sm font-semibold transition-colors ${stagiaires === n ? 'bg-[#195144] text-white' : 'bg-[#195144]/8 text-[#195144] hover:bg-[#195144]/15'}`}>
                {n}
              </button>
            ))}
          </div>
      ...
      {etape === 4 && (
        <div>
            ...
            {[{ v: false, l: "Aucune formation cette année" }, { v: true, l: 'Déjà formé cette année' }].map((o) => (
              <button key={String(o.v)} onClick={() => setDejaForme(o.v)}
                className={`rounded-2xl border px-4 py-3 text-sm font-medium transition-colors ${
                  dejaForme === o.v ? 'border-[#195144] bg-[#195144]/5 text-[#14110F]' : 'border-[#E7E5E4] text-[#57534E] hover:border-[#195144]/40'
                }`}>
                {o.l}
              </button>
            ))}
          ...
          {estimation && (
            <div className="rounded-2xl bg-[#195144] text-white p-6 md:p-7">
          ...
          <form onSubmit={envoyer} className="mt-5 space-y-3">
            ...
            <div className="grid sm:grid-cols-3 gap-2.5">
              <input name="nom" required placeholder="Votre nom" className="rounded-xl border border-[#E7E5E4] px-4 py-3 text-sm focus:outline-none focus:border-[#195144]/50" />
              <input name="email" type="email" placeholder="Email" className="rounded-xl border border-[#E7E5E4] px-4 py-3 text-sm focus:outline-none focus:border-[#195144]/50" />
              <input name="telephone" placeholder="Téléphone" className="rounded-xl border border-[#E7E5E4] px-4 py-3 text-sm focus:outline-none focus:border-[#195144]/50" />
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <button type="submit" disabled={envoi}
                className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-[#195144] text-white text-sm font-semibold hover:bg-[#123f34] disabled:opacity-50">
