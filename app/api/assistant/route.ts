...
export async function POST(req: Request) {
  ...
  const systeme = [
    ...
    `- Tu es en lecture seule : tu ne peux rien modifier ni envoyer. Si on te demande une action (envoyer un email, changer un statut), indique où le faire dans le CRM avec le lien.`,
  ...
  try {
    for (let tour = 0; tour < MAX_TOURS; tour++) {
      ...
      if (rep.stop_reason !== 'tool_use' || appels.length === 0) {
        ...
        return NextResponse.json({ reponse: texte || 'Je n’ai pas de réponse à te donner sur ce point.' })
      }
    ...
    return NextResponse.json({ reponse: 'La recherche est trop longue, reformule ta demande de façon plus précise.' })
  } catch (e: any) {
